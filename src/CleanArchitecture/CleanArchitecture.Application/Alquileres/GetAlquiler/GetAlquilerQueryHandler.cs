...
internal sealed class GetAlquilerQueryHandler : IQueryHandler<GetAlquilerQuery, AlquilerResponse>
{
    ...
    )
    {
        ...
        //Primero le paso el SCRIPT de SQL y despues le paso los parametros para la busqueda.
        var alquiler = await connection.QueryFirstOrDefaultAsync<AlquilerResponse>(sql, new {
            request.AlquilerId
        });
